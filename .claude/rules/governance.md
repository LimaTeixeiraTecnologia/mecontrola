...
- `agent-workflows-tools.md` — codifica o gate hard `R-AGENT-WF-001` para o substrato `internal/platform/agent` e consumidores como `internal/agents` (re-escopado pela emenda 2026-06-29; `internal/agent` descontinuado): fluxo `AgentRuntime.Execute -> Thread -> Run -> Agent.Execute -> tool`, comportamento novo como agente/tool/workflow no consumidor (sem `switch case intent.Kind`), Tool fina sem regra/SQL/branching, estados fechados (DMMF state-as-type), LLM so nas call-sites sancionadas, Run auditavel. Addendum `.6-A`/`.8-A` distingue mecanismo kernel de semantica agent. Addendum `.7-A` cobre estado de espera HITL: tipo fechado, persistencia antes da confirmacao, resume antes do parse, limpeza deterministica (gates literais SUPERSEDED ate reemissao pelo consumidor que reintroduzir HITL). Ver `.claude/rules/agent-workflows-tools.md`.
- `workflow-kernel.md` — codifica o gate hard `R-WF-KERNEL-001` para `internal/platform/workflow`: kernel generico sem import de dominio, sem regra/SQL/branching de dominio fora do adapter postgres, estados como tipos fechados (`RunStatus`/`StepStatus`/`SuspendReason`), cardinalidade controlada (sem `user_id`/`correlation_key`/`category_id`), LLM proibido. Gate bloqueante (ADR-004): redigido antes de qualquer codigo do kernel. Adicao de R-WF-KERNEL-001.7: contrato de resume via JSON merge-patch (RFC 7386), delta sobre snapshot, sem tipo de dominio exposto (ADR-001, prd-agent-platform-evolution). Ver `.claude/rules/workflow-kernel.md`.
