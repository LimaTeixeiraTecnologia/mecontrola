...
func (r *workerRuntime) newManager(ctx context.Context) (*worker.Manager, error) { //nolint:revive // composition root agrega bootstrap de módulos; refatorar fragmentaria a ordem de lifecycle crítica
	...
	agentsModule, err := agents.NewModule(agents.Deps{
		...
		Audio: agents.AudioConfig{
			Enabled:         r.cfg.AgentConfig.AudioEnabled,
			MetaAccessToken: r.cfg.WhatsAppConfig.AccessToken,
			STTModel:        r.cfg.AgentConfig.STTModel,
			STTTimeout:      r.cfg.AgentConfig.STTTimeout,
			MaxDuration:     r.cfg.AgentConfig.AudioMaxDuration,
			MaxBytes:        r.cfg.AgentConfig.AudioMaxBytes,
			MaxCostMicrousd: r.cfg.AgentConfig.AudioMaxCostMicrousd,
			MinConfidence:   r.cfg.AgentConfig.AudioMinConfidence,
			UncertainReply:  r.cfg.AgentConfig.AudioUncertainReply,
			RejectedReply:   r.cfg.AgentConfig.AudioRejectedReply,
		},
