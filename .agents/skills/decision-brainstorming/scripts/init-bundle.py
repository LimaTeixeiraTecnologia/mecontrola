#!/usr/bin/env python3
"""Inicializa a estrutura do bundle de brainstorming decisório.
...
def main() -> int:
    ...
    bundle_dir = root / "discoveries" / f"brainstorm-{slug}"
    ...
    (bundle_dir / "decision-brief.md").write_text(
        read_asset(script_dir, "decision-brief-template.md"),
        encoding="utf-8",
    )
    ...
    (bundle_dir / "assumptions.md").write_text(
        read_asset(script_dir, "assumptions-template.md"),
        encoding="utf-8",
    )
    (bundle_dir / "option-scorecard.md").write_text(
        read_asset(script_dir, "option-scorecard-template.md"),
        encoding="utf-8",
    )
