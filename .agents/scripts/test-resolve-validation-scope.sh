...
SCRIPT="$ROOT/.agents/scripts/resolve-validation-scope.sh"
FIXTURES_DIR="$ROOT/.agents/scripts/testdata/resolve-validation-scope"
...
run_scenario() {
  ...
  local output
  if [[ -f "$diff_file" ]]; then
    output="$(AGENTS_ROOT="$ROOT" bash "$SCRIPT" "${files[@]}" < "$diff_file")"
  else
    output="$(AGENTS_ROOT="$ROOT" bash "$SCRIPT" "${files[@]}")"
  fi
