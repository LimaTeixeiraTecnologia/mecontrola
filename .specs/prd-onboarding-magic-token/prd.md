...
2. O backend recebe `compra_aprovada` da Kiwify mas não tem como saber em qual conversa do WhatsApp o cliente vai aparecer, então a conta fica órfã até alguém intervir manualmente.
...
- **S-07 — Race entre `ATIVAR` e webhook `compra_aprovada` (Pix lento):** assume-se que casos em que o cliente envia `ATIVAR` antes do webhook chegar são minoritários e tratáveis por mensagem amigável ("pagamento ainda processando, tente em 1 minuto"). Se o volume desse caso se mostrar relevante em produção, pode justificar evolução posterior — fora deste PRD.
