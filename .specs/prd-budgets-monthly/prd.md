...
- **RF-60e**: O estado de cruzamento de cada limiar DEVE ser persistido em tabela dedicada `budgets_threshold_states` com chave `(user_id, competence, root_slug, threshold)` e colunas mínimas `currently_crossed` (boolean), `last_crossed_at` (timestamptz, UTC), `last_uncrossed_at` (timestamptz nullable, UTC), `version` (bigint monotônico incrementado a cada transição) e `last_evaluated_committed_at` (timestamptz, UTC). A transição DEVE ser atualizada exclusivamente pelo avaliador assíncrono em UPSERT idempotente.
- **RF-60f**: O avaliador DEVE ler `budgets_threshold_states` antes de decidir emissão; nova linha em `budgets_alerts` SOMENTE DEVE ser criada quando `currently_crossed` transitar de `false` para `true` no recálculo do estado financeiro atual da categoria. Evento de outbox cuja avaliação resulte em "permanece cruzado" ou "permanece abaixo" DEVE atualizar apenas `last_evaluated_committed_at` (ou nada) sem criar alerta.
...
- **RT-20**: Endpoints HTTP de budgets DEVEM viver sob o prefixo `/api/v1/budgets`; o contrato externo expõe `subcategory_id` em despesas e `category_id` (raiz) em alocações.
...
| Exposição de alertas | Tabela `budgets_alerts` + `GET /api/v1/budgets/alerts` (cursor); MVP sem `delivery_failed` |
...
| Estado de cruzamento de limiar | Tabela `budgets_threshold_states` com versão monotônica, atualizada apenas pelo avaliador assíncrono (RF-60e/RF-60f) |
