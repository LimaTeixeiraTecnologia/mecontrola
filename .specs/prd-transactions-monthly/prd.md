...
- v2 (2026-06-12): aplica 4 decisões de produto (edição cascateia sem aviso; job recorrência diário por `day_of_month`; `occurred_at` futuro aceito; `CardPurchase` sempre `outcome`) e 5 ajustes de robustez/eficiência (lock por `(template_id, ref_month)` no job diário; semântica futuro=realizado-antecipado; diff `ref_months_affected` em eventos de `CardPurchase`; debounce de recompute por `(user_id, ref_month)`; `Idempotency-Key` TTL cravado em 24h; semântica de rollback redefinida como "novas escritas param; drain prossegue"; runbook e base vazia formalizados).
- v2.1 (2026-06-12): padroniza prefixo de rotas para `/api/v1/...` (substitui `/v1/...` em RFs, OUT-07/OUT-13 e AS-08); contrato de evento `.v1` permanece intacto (sufixo de versão de payload).
- v2.2 (2026-06-12): incorpora 2 audit fixes que sangram para nível de produto — (RF-14) clamp `min(day, last_day_of_target_month)` em `closing_at`/`due_at` quando o dia natural não existe no mês destino (alinha à prática de cartões BR); (RF-32) descreve idempotência do `RecurringMaterializerJob` como **double-layer real** (advisory lock first-cut + PK race-final) substituindo "três camadas" anteriores. Demais 4 fixes (commands package, invoice delta, naming, mockery inventory) são internos à techspec.
-->
...
- **RF-01**: O sistema deve oferecer `POST /api/v1/transactions` que cria um lançamento avulso autenticado pelo `auth.Principal` injetado pelo `EstablishPrincipal` do `internal/identity`.
- **RF-02**: O payload de criação deve aceitar `direction` (entrada/saída), `payment_method` (`pix`, `ted`, `debit_in_account`, `debit_card`, `cash`, `boleto`), `amount_cents` (>0), `description`, `category_id`, `subcategory_id` opcional e `occurred_at`. A criação deve rejeitar `payment_method=doc` com `400 validation_error`. O sistema **aceita `occurred_at` no futuro sem restrição**; o lançamento é tratado como "realizado antecipado" (sem distinção de previsto vs realizado no MVP) e impacta `monthly_summary` da competência derivada do `occurred_at` desde o commit, mesmo que a data ainda não tenha chegado.
...
- **RF-05**: O sistema deve oferecer `PATCH /api/v1/transactions/{id}` que permite editar totalmente `direction`, `amount_cents`, `description`, `category_id`, `subcategory_id`, `payment_method` e `occurred_at` de um lançamento avulso, atualizando `ref_month` quando `occurred_at` mudar de mês.
- **RF-06**: O sistema deve oferecer `GET /api/v1/transactions/{id}` que retorna o lançamento avulso pelo seu identificador, verificando propriedade pelo `user_id` do principal; recurso de outro usuário retorna `404 not_found`.
- **RF-07**: O sistema deve oferecer `GET /api/v1/transactions?ref_month=YYYY-MM&cursor=&limit=` que lista lançamentos avulsos do usuário no mês informado, paginado por cursor base64(`created_at`,`id`) descendente, limit padrão 50, máximo 200.
- **RF-08**: O sistema deve oferecer `DELETE /api/v1/transactions/{id}` que aplica soft-delete (`deleted_at = now`), preservando trilha de auditoria.
...
- **RF-11**: O sistema deve oferecer `POST /api/v1/card-purchases` que cria uma compra parcelada de cartão de crédito com `card_id`, `total_amount_cents` (>0), `installments_total` (1 a 24), `description`, `category_id`, `subcategory_id` opcional, `purchased_at`. `CardPurchase` é sempre `direction=outcome` no MVP; estorno, cashback e ajuste de fatura ficam fora do escopo (ver OUT-16) e devem ser registrados como `Transaction` avulsa de `income`.
...
- **RF-14**: O sistema deve calcular a competência da primeira parcela como segue: se `purchased_at.day <= card_closing_day`, a primeira fatura é o mês corrente vencendo no `card_due_day` do mês corrente; senão, a próxima fatura. As parcelas seguintes incrementam um mês cada. **Edge case — clamp para fim de mês**: quando o `card_closing_day` ou `card_due_day` apontar para um dia que não existe no mês destino (ex.: `card_due_day=30` em fevereiro, ou `card_closing_day=31` em meses de 30 dias), o sistema usa `min(day, last_day_of_target_month)` para `closing_at` e `due_at`. Política alinhada à prática de cartões brasileiros (fatura cai no último dia do mês quando o dia natural inexiste); evita data inválida e overflow para o mês seguinte.
...
- **RF-17**: O sistema deve oferecer `PATCH /api/v1/card-purchases/{id}` que permite editar `total_amount_cents`, `installments_total`, `description`, `category_id`, `subcategory_id`, `purchased_at`. A edição deve recriar/atualizar todas as parcelas em cascata, inclusive em faturas já fechadas, dentro da mesma transação, **sem aviso ou confirmação adicional do cliente** (decisão de produto: edição retroativa silenciosa). O response deve incluir `ref_months_affected: []string` com a união das competências antigas e novas para que o cliente possa atualizar telas em cache.
- **RF-18**: O sistema deve oferecer `DELETE /api/v1/card-purchases/{id}` que aplica soft-delete na compra-pai e em todas as parcelas vinculadas em uma única transação.
- **RF-19**: O sistema deve oferecer `GET /api/v1/card-purchases/{id}` que retorna a compra com a lista de parcelas (`installment_index`, `ref_month`, `amount_cents`, `invoice_id`).
- **RF-20**: O sistema deve oferecer `GET /api/v1/card-purchases?card_id=&ref_month=&cursor=&limit=` que lista compras paginadas por cursor.
- **RF-21**: O sistema deve oferecer `GET /api/v1/cards/{card_id}/invoices/{ref_month}` que retorna a fatura simulada do cartão no mês informado com `items_total_cents`, `closing_at`, `due_at` e a lista de itens.
...
- **RF-23**: O sistema deve oferecer `GET /api/v1/months/{ref_month}` que retorna `income_cents`, `outcome_cents`, `total_cents` e `updated_at` da projeção `monthly_summary` para o usuário autenticado.
- **RF-24**: O sistema deve oferecer `GET /api/v1/months/{ref_month}/entries?cursor=&limit=` que lista o extrato unificado do mês combinando lançamentos avulsos e itens de fatura, paginado por cursor base64(`created_at`,`id`) descendente.
- **RF-25**: Toda mutação confirmada (commit) em `Transaction`, `CardPurchase` ou `CardInvoiceItem` deve causar recálculo eventual da projeção `monthly_summary` para todas as competências afetadas (a anterior e a nova, quando `ref_month` mudar). Em `CardPurchase`, o conjunto de competências afetadas vem do array `ref_months_affected` do evento (RF-36).
- **RF-26**: O consumer de recálculo deve ser idempotente: reprocessar o mesmo evento ou disparar recálculo paralelo da mesma chave (`user_id`, `ref_month`) não pode corromper `income_cents`, `outcome_cents` ou `total_cents`. O consumer deve aplicar **debounce/coalescing** por chave `(user_id, ref_month)` em janela configurável (`OutboxConfig.MonthlySummaryDebounceWindow`, default 1500 ms): múltiplos eventos da mesma chave dentro da janela colapsam em um único recompute, preservando exactly-once semântico do resultado. A janela é métrica observada (`transactions_monthly_summary_coalesce_factor`) e nunca pode exceder o SLO da RT-12 (p95 < 5 s).
...
- **RF-28**: Caso `monthly_summary` ainda não tenha sido projetado, `GET /api/v1/months/{ref_month}` deve responder `200` com totais zerados e `updated_at=null`, nunca `404`.

### Lançamento recorrente (RecurringTemplate)

- **RF-29**: O sistema deve oferecer `POST /api/v1/recurring-templates` que cria um template recorrente com `direction`, `payment_method`, `card_id` opcional (obrigatório quando `payment_method=credit_card`), `amount_cents`, `description`, `category_id`, `subcategory_id` opcional, `frequency` (`monthly`/`yearly`), `day_of_month` (1 a 28), `started_at`, `ended_at` opcional, `installments_total` (≥1, ≤24, default 1).
- **RF-30**: O sistema deve oferecer `PATCH /api/v1/recurring-templates/{id}`, `DELETE /api/v1/recurring-templates/{id}`, `GET /api/v1/recurring-templates/{id}` e `GET /api/v1/recurring-templates?cursor=&limit=&active=true|false`.
- **RF-31**: Um job **diário** `RecurringMaterializerJob` deve materializar, a cada execução, todos os templates ativos (sem `ended_at` ou com `ended_at >= today`) cujo `day_of_month` seja igual ao dia corrente em `America/Sao_Paulo` para a competência corrente, criando o `Transaction` ou `CardPurchase` correspondente com `occurred_at = today 12:00 America/Sao_Paulo`. Templates criados após o `day_of_month` do mês corrente só serão materializados no mês seguinte (sem retroatividade automática).
- **RF-32**: A materialização deve ser idempotente em **duas camadas reais**: (a) **first-cut** — o handler tenta `pg_try_advisory_xact_lock` por `(template_id, ref_month)`; se não adquirir, emite `transactions_recurring_materialize_skipped_total{reason="lock_not_acquired"}` e sai sem trabalho; (b) **race-final** — a tabela `transactions_recurring_materializations` usa `(template_id, ref_month)` como `PRIMARY KEY` e o INSERT usa `ON CONFLICT DO NOTHING`; conflito incrementa `transactions_recurring_materialize_skipped_total{reason="already_materialized"}` sem erro. O job emite ainda `transactions_recurring_materialize_attempt_total` e `transactions_recurring_materialize_duration_seconds`.
...
- **RF-36**: Toda criação, edição e exclusão lógica de `CardPurchase` deve gravar UM ÚNICO evento `transactions.card_purchase.{created|updated|deleted}.v1` contendo o array completo de parcelas e o array `ref_months_affected: []string` (união das competências da versão anterior e da versão nova) no payload, na mesma transação do agregado. Consumidores devem usar `ref_months_affected` para invalidar todas as projeções afetadas em vez de inferir do payload de parcelas.
...
- **RF-47**: Alteração da flag `transactions.enabled` para `false` deve **descontinuar o caminho de novas escritas em < 5 minutos** via reload de configuração no `cmd/api`. A semântica é "stop new writes", não "stop world": mutações já em flight completam, o `outbox` continua drenando eventos pendentes até esgotar a fila, e jobs em execução finalizam o ciclo corrente. Para parada total imediata, é necessário deploy de rollback (fora do SLO de 5 minutos).
...
- **RT-07**: Idempotência via `internal/platform/idempotency.Middleware` com `scope="transactions"` e TTL fixado em **24 h** (`configs.TransactionsConfig.IdempotencyTTL=24h`). Janela menor causaria 409 em replays legítimos de clientes mobile com perda de conectividade; janela maior bloquearia reuso acidental de key. Valor é overridable por env apenas em ambiente de teste.
...
- **OUT-07**: API GraphQL; apenas REST sob prefixo `/api/v1`.
...
- **OUT-13**: Rate-limit dedicado para `POST /api/v1/card-purchases`; relies em rate-limit global existente.
...
- **OUT-16**: `CardPurchase` com `direction=income` (estorno, cashback, ajuste de fatura). Operacionalmente o usuário registra como `Transaction` avulsa de `income` sem vínculo com a fatura simulada; reflexo correto no resumo mensal, sem reflexo na visão de fatura. Backlog para v2 (item de fatura negativo + lifecycle de estorno).
- **OUT-17**: Re-projeção de competência após mudança de `closing_day`/`due_day` no `internal/card`: snapshot estático em `CardPurchase` (RT-20) é deliberadamente imutável e não retroage. Para corrigir, usuário edita a `CardPurchase` (RF-17), que recalcula competências usando o snapshot vigente naquela compra. Backlog para v2 considera evento `card.billing_cycle_changed.v1`.
- **OUT-18**: Distinção previsto vs realizado em `Transaction` com `occurred_at` futuro. MVP trata como "realizado antecipado" e impacta `monthly_summary` desde o commit (RF-02). Backlog para v2 considera campo `posting_date` separado de `occurred_at` para postergar projeção até a data real.
...
- **AS-08**: Avaliar necessidade de rate-limit por usuário em `POST /api/v1/card-purchases` se observação de produção mostrar abuso com 24 parcelas.
...
- **AS-11**: MVP assume **base de produção vazia** no go-live do módulo `internal/transactions`. Não há backfill retroativo de `monthly_summary` para usuários pré-existentes do `internal/identity`. Se houver usuários com `Card` cadastrado antes do go-live, eles começam com `monthly_summary` zerado e a projeção materializa naturalmente na primeira mutação. A confirmar com produto antes da técspec se a premissa de base vazia não se sustentar.
- **AS-12**: Runbook operacional dedicado em `docs/runbooks/transactions.md` deve cobrir, no mínimo, três cenários antes do go-live: (a) consumer travado em loop de retry (drenar manual + reset de cursor), (b) drift detectado pelo job diário > limiar (investigar evento perdido), (c) dead-letter > 0 (replay manual ou descarte com auditoria). Endpoints administrativos permanecem fora de escopo (OUT-09); operação é via SQL + ferramentas existentes.
