...
|---|--------|--------|--------------|---------------|--------|
| 1.0 | Migrations 000004/000005 e schema `mecontrola.cards` + `idempotency_keys` | pending | — | — | — |
| 2.0 | Pacote `internal/platform/idempotency/` (Storage + Middleware + Recorder) | pending | 1.0 | — | — |
| 3.0 | Domain do `card` — VOs, agregado, `InvoiceFor` puro, fixtures + property-based | pending | — | Com 6.0 | — |
| 4.0 | Application layer — DTOs, interfaces, use cases (UoW + Storage), mocks | pending | 2.0, 3.0 | — | — |
| 5.0 | Repository pgx + paginação keyset + mapping de erros + integration tests | pending | 1.0, 4.0 | Com 7.0 | — |
| 6.0 | Identity additive — `auth.SourceHeader` + `InjectPrincipalFromHeader` middleware | pending | — | Com 3.0 | — |
| 7.0 | HTTP layer do `card` — handlers thin, router com chain, redact helper, spans/logs | pending | 2.0, 4.0, 6.0 | Com 5.0 | — |
| 8.0 | Contrato OpenAPI 3.1 + contract tests (kin-openapi) + Postman collection | pending | 7.0 | — | postman-collection-generator |
| 9.0 | Wiring + governança operacional (module.go, server.go, lint anti-PCI, runbook, dashboard) | pending | 5.0, 7.0, 8.0 | — | otel-grafana-dashboards, taskfile-production |
| 10.0 | Load test k6 + evidência de SLO M-02/M-03/M-04 | pending | 9.0 | — | — |
