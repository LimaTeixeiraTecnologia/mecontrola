...
func (uc *CreateRecurringTemplate) Execute(ctx context.Context, raw input.RawCreateRecurringTemplate) (output.RecurringTemplate, error) {
	...
	rawCmd, rawErr := buildRawCreateRecurringTemplate(raw)
	if rawErr != nil {
		return output.RecurringTemplate{}, fmt.Errorf("transactions/create_recurring_template: %w", rawErr)
	}
	...
	evidence, gateErr := approveUpdateCategory(ctx, uc.categoryGate, categoryEvidence{}, cmd.Direction.String(), "create_recurring_template", cmd.CategoryID.UUID(), catSubID)
	if gateErr != nil {
		span.RecordError(gateErr)
		return output.RecurringTemplate{}, fmt.Errorf("transactions/create_recurring_template: gate de categoria: %w", gateErr)
	}
...
func buildRawCreateRecurringTemplate(raw input.RawCreateRecurringTemplate) (commands.RawCreateRecurringTemplate, error) {
	startedAt, parseErr := parseISO8601(raw.StartedAt)
	if parseErr != nil {
		return commands.RawCreateRecurringTemplate{}, fmt.Errorf("started_at inválido: %w", parseErr)
	}
	...
	if raw.EndedAt != nil {
		...
		if endErr != nil {
			return commands.RawCreateRecurringTemplate{}, fmt.Errorf("ended_at inválido: %w", endErr)
		}
