...
func NewUpdateRecurringTemplate(raw RawUpdateRecurringTemplate, userID uuid.UUID) (UpdateRecurringTemplate, error) { //nolint:revive // smart constructor valida 10+ campos; complexidade é estrutural, não acidental
	var errs []error

	templateID, err := uuid.Parse(raw.TemplateID)
	if err != nil {
		errs = append(errs, fmt.Errorf("template_id: %w", err))
	}
	...
	if len(errs) > 0 {
		return UpdateRecurringTemplate{}, fmt.Errorf("commands/update_recurring_template: %w", errors.Join(errs...))
	}
