...
type ActivateBudgetCommandSuite struct {
	suite.Suite
}

func TestActivateBudgetCommandSuite(t *testing.T) {
	suite.Run(t, new(ActivateBudgetCommandSuite))
}

func (s *ActivateBudgetCommandSuite) TestNewActivateBudgetCommand() {
	...
	for _, tc := range cases {
		s.Run(tc.name, func() {
			_, err := commands.NewActivateBudgetCommand(tc.userID, tc.competence)
