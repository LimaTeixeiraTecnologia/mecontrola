...
type DeleteDraftBudgetCommandSuite struct {
	suite.Suite
}

func TestDeleteDraftBudgetCommandSuite(t *testing.T) {
	suite.Run(t, new(DeleteDraftBudgetCommandSuite))
}

func (s *DeleteDraftBudgetCommandSuite) TestNewDeleteDraftBudgetCommand() {
	...
	for _, tc := range cases {
		s.Run(tc.name, func() {
			_, err := commands.NewDeleteDraftBudgetCommand(tc.userID, tc.competence)
