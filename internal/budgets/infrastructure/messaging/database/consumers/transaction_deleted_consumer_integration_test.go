...
type TransactionDeletedConsumerIntegrationSuite struct {
	suite.Suite
}

func TestTransactionDeletedConsumerIntegration(t *testing.T) {
	suite.Run(t, new(TransactionDeletedConsumerIntegrationSuite))
}

func (s *TransactionDeletedConsumerIntegrationSuite) newModule(db *sqlx.DB) *budgets.BudgetsModule {
...
func (s *TransactionDeletedConsumerIntegrationSuite) seedExpense(ctx context.Context, mod *budgets.BudgetsModule, userID uuid.UUID, aggregateID string) {
...
func (s *TransactionDeletedConsumerIntegrationSuite) buildDeletedEnvelope(aggregateID, userID string) outbox.Envelope {
	raw, _ := json.Marshal(map[string]any{
		"aggregate_id": aggregateID,
		"user_id":      userID,
		"occurred_at":  time.Date(2026, 6, 17, 12, 0, 0, 0, time.UTC),
...
func (s *TransactionDeletedConsumerIntegrationSuite) TestSoftDeletesExpense() {
	...
	s.seedExpense(ctx, mod, userID, aggregateID)

	env := s.buildDeletedEnvelope(aggregateID, userID.String())
	evt := stubEvent{eventType: "transactions.transaction.deleted.v1", payload: env}
	s.Require().NoError(mod.TransactionDeletedConsumer.Handle(ctx, platformevents.Event(evt)))
...
func (s *TransactionDeletedConsumerIntegrationSuite) TestIdempotency() {
	db, _ := testcontainer.Postgres(s.T())
	mod := s.newModule(db)
	ctx := context.Background()
	...
	var deletedAt1 *time.Time
	s.Require().NoError(db.QueryRowContext(ctx,
		`SELECT deleted_at FROM mecontrola.budgets_expenses WHERE user_id = $1 AND external_transaction_id = $2`,
		userID.String(), aggregateID,
	).Scan(&deletedAt1))
	s.Require().NotNil(deletedAt1)

	err2 := mod.TransactionDeletedConsumer.Handle(ctx, platformevents.Event(evt))
	...
	s.Require().NotNil(deletedAt2)
	s.Equal(deletedAt1.UTC().Truncate(time.Second), deletedAt2.UTC().Truncate(time.Second))
}
