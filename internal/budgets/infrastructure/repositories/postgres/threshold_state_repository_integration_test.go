...
func (s *ThresholdStateRepositorySuite) TestInsertNewRow_Transitioned() {
	mgr := setupTestDB(s.T())
	repo := newThresholdStateRepo(testO11y())
	...
	transitioned, err := repo.UpsertIfTransition(ctx, db, key, true, committedAt)
...
func (s *ThresholdStateRepositorySuite) TestFalseToTrue_Transitioned() {
	...
	trans1, err := repo.UpsertIfTransition(ctx, db, key, false, t1)
	s.Require().NoError(err)
	s.Assert().True(trans1)

	t2 := t1.Add(time.Second)
	trans2, err := repo.UpsertIfTransition(ctx, db, key, true, t2)
	s.Require().NoError(err)
...
func (s *ThresholdStateRepositorySuite) TestTrueToFalse_Transitioned() {
	...
	_, err := repo.UpsertIfTransition(ctx, db, key, true, t1)
	s.Require().NoError(err)

	t2 := t1.Add(time.Second)
	transitioned, err := repo.UpsertIfTransition(ctx, db, key, false, t2)
	s.Require().NoError(err)
...
func (s *ThresholdStateRepositorySuite) TestSameState_NotTransitioned() {
	...
	t2 := t1.Add(time.Second)
	transitioned, err := repo.UpsertIfTransition(ctx, db, key, true, t2)
	s.Require().NoError(err)
...
func (s *ThresholdStateRepositorySuite) TestOutOfOrder_Ignored() {
	...
	_, err := repo.UpsertIfTransition(ctx, db, key, true, t2)
	s.Require().NoError(err)

	transitioned, err := repo.UpsertIfTransition(ctx, db, key, false, t1)
	s.Require().NoError(err)
...
func (s *ThresholdStateRepositorySuite) TestIdempotent_SameTimestamp() {
	...
	trans1, err := repo.UpsertIfTransition(ctx, db, key, true, committedAt)
	s.Require().NoError(err)

	trans2, err := repo.UpsertIfTransition(ctx, db, key, false, committedAt)
	s.Require().NoError(err)
