...
func (s *BudgetRepositorySuite) TestCreateAndGetDraft() {
	...
	s.Require().NoError(repo.CreateDraft(ctx, db, budget))

	found, err := repo.GetByUserCompetence(ctx, db, userID, competence)
...
func (s *BudgetRepositorySuite) TestGetByUserCompetenceNotFound() {
	...
	_, err := repo.GetByUserCompetence(ctx, db, uuid.New(), mustCompetence(s.T(), "2025-06"))
	s.Require().Error(err)
...
func (s *BudgetRepositorySuite) TestActivateBudget() {
	...
	s.Require().NoError(repo.Activate(ctx, db, budget))
...
func (s *BudgetRepositorySuite) TestDeleteDraft() {
	...
	s.Require().NoError(repo.CreateDraft(ctx, db, budget))
	s.Require().NoError(repo.DeleteDraft(ctx, db, userID, competence))

	_, err := repo.GetByUserCompetence(ctx, db, userID, competence)
	s.Assert().True(errors.Is(err, interfaces.ErrBudgetNotFound))
}
...
func (s *BudgetRepositorySuite) TestDeleteDraftNotFound() {
	...
	err := repo.DeleteDraft(ctx, db, uuid.New(), mustCompetence(s.T(), "2025-04"))
	s.Require().Error(err)
	s.Assert().True(errors.Is(err, interfaces.ErrBudgetNotFound))
}
...
func (s *BudgetRepositorySuite) TestListFutureNotActivated() {
	...
	for _, comp := range []string{"2025-05", "2025-06", "2025-07"} {
		...
		s.Require().NoError(repo.CreateDraft(ctx, db, b))
	}

	from := mustCompetence(s.T(), "2025-05")
	results, err := repo.ListFutureNotActivated(ctx, db, userID, from, 10)
	s.Require().NoError(err)
