	...
	scenarios := []struct {
	...
	}{
		{
			...
			setup: func() args {
				...
				return args{userID: userID, channel: "whatsapp", externalID: "+5511911111111"}
			},
		...
		},
		{
			...
			setup: func() args {
				...
				return args{userID: userID, channel: "whatsapp", externalID: "+5511922222222"}
			},
			expect: func(res usecases.LinkChannelResult, err error, a args) {
				s.Require().NoError(err)
		...
		},
		{
			...
			setup: func() args {
				...
				_, err := sut.Execute(s.ctx, input.LinkChannelToUser{
					UserID:     userIDA,
					Channel:    "whatsapp",
					ExternalID: "+5511933333333",
				})
				s.Require().NoError(err)

				return args{userID: userIDB, channel: "whatsapp", externalID: "+5511933333333"}
			},
			expect: func(res usecases.LinkChannelResult, err error, a args) {
				s.Require().Error(err)
