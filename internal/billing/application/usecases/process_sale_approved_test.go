...
func (s *ProcessSaleApprovedSuite) TestFunnelTokenVazioRetornaErro() {
	in := s.validInput()
	in.FunnelToken = ""

	err := s.uc.Execute(context.Background(), in)
	s.Require().Error(err)
	s.True(errors.Is(err, usecases.ErrFunnelTokenMissing))
}
...
func (s *ProcessSaleApprovedSuite) TestIdempotencia5xRetorna1TransicaoE4ErrAlreadyProcessed() {
	...
	s.publisherMock.On("PublishActivated", mock.Anything, mock.Anything, mock.Anything, "sub-001", "token-abc").Return(nil).Once()
