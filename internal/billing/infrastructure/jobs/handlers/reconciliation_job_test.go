	...
	scenarios := []struct {
		...
		},
		{
			...
			setup: func(ctx context.Context) {
				...
				s.eventRepoMock.EXPECT().MarkApplied(ctx, "order_refunded:sale-refund-01", "order_refunded", "sale-refund-01", saleTime).Return(nil).Once()
		...
		},
		{
			...
			setup: func(ctx context.Context) {
				...
				s.factoryMock.EXPECT().ProcessedEventRepository(nil).Return(s.eventRepoMock).Once()
				s.factoryMock.EXPECT().SubscriptionRepository(nil).Return(s.subRepoMock).Once()
				s.eventRepoMock.EXPECT().MarkApplied(ctx, "order_refunded:sale-idem-01", "order_refunded", "sale-idem-01", saleTime).
