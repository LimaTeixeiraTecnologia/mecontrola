...
func buildAllFakeTools(userID string) ([]tool.ToolHandle, map[string]**captureResult) {
	...
	tools := []tool.ToolHandle{
		...
		buildCaptureTool("list_cards", "Lista os 💳 cadastrados do usuário; use apenas quando o usuário pedir explicitamente para ver, listar ou saber quais são seus 💳, nunca como etapa preparatória de um registro de compra ou consulta de fatura", baseSchema(),
			func() **captureResult {
				var c *captureResult
				captures["list_cards"] = &c
				return &c
			}()),
		buildCaptureTool("get_card", "Busca dados de um 💳 pelo ID", baseSchema("cardId"), makeCapture("get_card")),
		buildCaptureTool("count_cards", "Conta 💳 do usuário", baseSchema(), makeCapture("count_cards")),
		buildCaptureTool("best_purchase_day", "Calcula melhor dia de compra", baseSchema("bank", "dueDay"), makeCapture("best_purchase_day")),
		buildCaptureTool("query_card_invoice", "Consulta fatura do 💳", baseSchema("cardId", "refMonth"), makeCapture("query_card_invoice")),
		buildCaptureTool("list_recurrences", "Lista templates de recorrência", baseSchema(), makeCapture("list_recurrences")),
		buildCaptureTool("update_recurrence", "Solicita atualização de recorrência", baseSchema("templateId", "version", "description"), makeCapture("update_recurrence")),
		buildCaptureTool("delete_recurrence", "Solicita exclusão de recorrência", baseSchema("templateId", "version"), makeCapture("delete_recurrence")),
...
func TestRealLLM_ToolCoverage_All22Tools(t *testing.T) {
	...
	scenarios := []harnessScenario{
		...
		{input: "quais são meus 💳?", expectedTool: "list_cards", tools: tools},
		{input: "me mostra os dados do 💳 id cartao-001", expectedTool: "get_card", tools: tools},
		{input: "quantos 💳 eu tenho?", expectedTool: "count_cards", tools: tools},
		{input: "qual é o melhor dia para comprar no banco nubank com vencimento dia 10?", expectedTool: "best_purchase_day", tools: tools},
		{input: "qual é a fatura do meu 💳 cartao-001 em julho 2026?", expectedTool: "query_card_invoice", tools: tools},
		...
		{input: "quero atualizar o apelido do 💳 id cartao-001 para nubank pessoal", expectedTool: "update_card", tools: tools},
		...
		{input: "quanto está minha fatura do 💳 nubank?", expectedTool: "resolve_card", tools: tools},
