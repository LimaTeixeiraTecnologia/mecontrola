...
func BuildUpdateCardTool(engine wf.Engine[workflows.ConfirmState], def wf.Definition[workflows.ConfirmState], cards interfaces.CardManager) tool.ToolHandle {
	...
	return tool.NewTool[UpdateCardInput, UpdateCardOutput]("update_card", "Atualiza os dados de um cartão. Requer confirmação quando o dia de vencimento é alterado.", in, out, exec)
}
