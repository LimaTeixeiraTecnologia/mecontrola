...
func formatBudgetDetails(out budgetsoutput.MonthlySummaryOutput) string {
	var sb strings.Builder
	sb.WriteString("📋 *Detalhes do orçamento ")
	sb.WriteString(out.Competence)
	sb.WriteString("*\n")
	if out.TotalPlannedCents != nil {
		sb.WriteString("Planejado total: ")
		sb.WriteString(formatBRL(*out.TotalPlannedCents))
		sb.WriteString("\n")
	}
	sb.WriteString("Gasto total: ")
	sb.WriteString(formatBRL(out.TotalSpentCents))
	if out.PercentageTotal != nil {
		_, _ = fmt.Fprintf(&sb, " (%.0f%%)", *out.PercentageTotal)
	}
	rendered := false
	for _, allocation := range out.Allocations {
		if allocation.PlannedCents == nil && allocation.SpentCents == 0 {
			continue
		}
		if !rendered {
			sb.WriteString("\n")
	...
	if !rendered && out.TotalPlannedCents == nil {
		sb.WriteString("\nVocê ainda não tem um planejamento definido pra esse mês. Quer configurar um agora? 🙂")
	}
	return sb.String()
}

func formatCategoryList(r CategoryListResult) string {
	if len(r.Categories) == 0 {
		return "📂 Você ainda não tem categorias cadastradas."
	}
	var sb strings.Builder
	_, _ = fmt.Fprintf(&sb, "📂 *Suas categorias* (%d)\n", len(r.Categories))
	for _, category := range r.Categories {
		...
		sb.WriteString("• ")
		sb.WriteString(name)
		if parent := strings.TrimSpace(category.ParentSlug); parent != "" {
			sb.WriteString(" — ")
			sb.WriteString(rootSlugLabel(parent))
		}
		sb.WriteString("\n")
	}
	return strings.TrimRight(sb.String(), "\n")
...
func formatCategoryClassification(r CategoryClassifyResult) string {
	name := strings.TrimSpace(r.Name)
	if name == "" {
		name = strings.TrimSpace(r.Slug)
	}
	path := strings.TrimSpace(r.Path)
	if path != "" {
		return fmt.Sprintf("📂 Isso entra em *%s* (%s).", name, path)
	}
	return fmt.Sprintf("📂 Isso entra em *%s*.", name)
}

func formatClassifyNotFound(query string) string {
	trimmed := strings.TrimSpace(query)
	if trimmed == "" {
		return "Não encontrei uma categoria pra isso. Pode me dar mais detalhes? 🙂"
	}
	return fmt.Sprintf("Não encontrei categoria para %q. Pode reformular ou me dizer outro item? 🙂", trimmed)
}
