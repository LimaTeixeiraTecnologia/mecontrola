...
- Objetivo: cobrir 100% das rotas montadas (57) e simular a jornada real encadeada
...
`card_purchase_id`/`_version`, `recurring_id`/`_version`, `expense_id`/`_version`, `ref_month`,
...
- **Gateway** — canonical `lower(user_id) + "." + timestamp` (unix segundos); `HMAC-SHA256` hex;
  **a chave é o `gateway_secret` hex-decodificado** (`CryptoJS.enc.Hex.parse`); janela 60s; headers
  `X-User-ID`, `X-Gateway-Timestamp`, `X-Gateway-Auth`. Mutáveis de transactions/cards/card-purchases/
...
4. `10 — Jailton` e `20 — Stefany`:
   ...
   - `C — App autenticado`: categorias → transações → cartões → compras no cartão → recorrências → orçamentos. O pre-request da pasta assina cada request e calcula datas/competências.
5. `99 — Regressão negativa` — webhook Kiwify com secret errado → 401.

## Enums/invariantes usados (verbatim do domínio, evitam 400/409)

- Transação: `direction` ∈ {income, outcome}; `payment_method` ∈ {pix, ted, debit_in_account, debit_card, cash, boleto, credit_card} (`doc` é read-only); `amount_cents > 0`; subcategoria obrigatória em `outcome`.
- Card purchase: `installments_total` 1..24. Recorrência: `frequency` ∈ {monthly, yearly}, `day_of_month` 1..28.
...
## Cobertura — 57/57 rotas montadas
...
| Transactions (tx CRUD 5, card-purchases CRUD 5, recurring CRUD 5, months 2) | 17 | 10/20 C |

Total de rotas distintas cobertas: **57** (auditado por `method + path`). Total de requisições na collection: **112** (rotas autenticadas repetidas nos 2 perfis).

## Validação executada

- JSON válido (`python3 -m json.tool`) para collection e environment.
- `schema` = Postman Collection v2.1.
- Auditoria automática: 57 pares `method+path` distintos == inventário de rotas montadas.
