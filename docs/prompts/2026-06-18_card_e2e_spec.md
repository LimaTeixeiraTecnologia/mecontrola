# Prompt Refinado: Implementação de Testes E2E para o Módulo card

## Contexto
Você é um Engenheiro Go Sênior. Sua tarefa é projetar e planejar a implementação de 100% de cobertura de testes E2E para o módulo `internal/card`, garantindo que ele atenda aos mais altos padrões arquiteturais. Você deve basear seu trabalho nos padrões encontrados em `internal/billing` e `internal/e2e`, particularmente no que diz respeito a fluxos orientados a eventos e validação de outbox.

> **Expectativa de entrega:** fechar **100% do módulo `internal/card` com confiança operacional**. "Confiança operacional" significa que cada camada possui um teste que **falha de verdade** quando o comportamento prometido quebra — não basta o teste existir e passar; ele precisa exercitar o caminho real (HTTP, producer, consumer, job, repositório) e **verificar o estado do banco** após a operação.

## Objetivos
1.  **Cobertura Exaustiva:** Identificar e planejar todos os cenários E2E possíveis para `internal/card`, incluindo:
...
Cada linha abaixo é um conjunto mínimo de cenários que DEVE existir antes de declarar o módulo coberto. Adapte os caminhos `card` à realidade descoberta no "Inventário a Descobrir".
...
Mapeie o módulo real `internal/card` e liste explicitamente — cobertura "100%" sem este inventário é cega:
...
3.  **Estrutura de Pastas:** Proponha uma organização limpa sob `internal/card/e2e` ou `internal/e2e`.
...
Funcionalidade: [NOME DA FUNCIONALIDADE]
  ...
  Cenário: [DESCRIÇÃO DO CENÁRIO]
    Dado que o ambiente de teste para card está pronto
...
```gherkin
# language: pt
Funcionalidade: Persistência e propagação de eventos em card
